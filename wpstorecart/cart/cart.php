		...
		if ($valid_item_qty !== false && $valid_item_price !== false)
			...
			$this->_update_total();
                        ...
                        if(!$is_multi_add && $wpStoreCartOptions['redirect_to_checkout']=='true' && $wpStoreCartOptions['checkoutpageurl']!='' ) {
                            if (!headers_sent()) {
                                header('Location: '.$wpStoreCartOptions['checkoutpageurl']);
                                exit;
                            } else {
                                echo '
                                <script type="text/javascript">
                                /* <![CDATA[ */
                                window.location = "'.$wpStoreCartOptions['checkoutpageurl'].'"
                                /* ]]> */
                                </script>
                                ';          
                                exit;
		...
		if (isset($error_message)) {
			$output .= "\t$error_message\n";
                ...
                if ($is_checkout == true) {
                    if ( is_user_logged_in() ) {
                     ...
                     if($isLoggedIn == false && $wpStoreCartOptions['requireregistration']!='disable') {
                            ...
                            <br />
                            <form name="wpsc-registerform" id="wpsc-registerform" action="'.plugins_url().'/wpstorecart/wpstorecart/registration/register.php" method="post">
                    ...
                    if($alreadyDisplayedForm == false) {
                        foreach ($fields as $field) {
                            $specific_items = explode("||", $field['value']);
                                $contactmethods[wpscSlug($specific_item[0])] = $specific_item[0];
                ...
                if( $isLoggedIn == true || $is_checkout==false) {
                    ...
                    if($wpStoreCartOptions['storetype']!='Digital Goods Only' && (($wpStoreCartOptions['displayshipping']=='true' && $wpscWidgetSettings['iswidget']!='true')|| $wpscWidgetSettings['widgetShowShipping']=='true') ) {
                        $output .= '<div id="wpsc-shipping-calculation-form'; if(isset($wpscWidgetSettings)) {$output .= '-widget';} $output .='">';
                        $output .= '<span id="wpsc-shipping">'.$text['shipping'].': ';
                        $output .= '<select name="wpsc-shipping-dropdown" >';
                        $output .= wpscShippingAPICheckoutDropdown($this->get_contents());
                    ...
                    if(($wpStoreCartOptions['displaytaxes']=='true' && $wpscWidgetSettings['iswidget']!='true') || $wpscWidgetSettings['widgetShowTax']=='true' ) {
                        if($taxstates || $taxcountries) {
                            ...
                            if($wpStoreCartOptions['show_price_to_guests']=='false' && !is_user_logged_in()) {
                                $output .= "\t\t\t\t\t\t<span id='wpsc-tax"; if(isset($wpscWidgetSettings)) {$output .= '-widget';} $output .="'>" . $text['tax'] . ": <strong>" . $text['currency_symbol'] . $wpStoreCartOptions['logged_out_price'] . $text['currency_symbol_right'] ."</strong> (".$mastertax."%)</span><br />\n";
                            } else {
